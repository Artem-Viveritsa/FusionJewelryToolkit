...
from ...helpers.Gemstones import extractGemstonesInfo, findValidConnections, isGemstone
...
_uniformityValueInput: adsk.core.ValueCommandInput = None
...
DEFAULT_UNIFORMITY = 0.5
LEGACY_UNIFORMITY = 0.0


RESOURCES_FOLDER = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'resources', '')

createCommandInputDef = constants.InputDef(constants.ProngsBetweenGemstones.createCommandId, 'Create Prongs Between Gemstones', 'Creates prongs at the midpoint between nearby gemstones based on distance constraint.')
editCommandInputDef = constants.InputDef(constants.ProngsBetweenGemstones.editCommandId, 'Edit Prongs Between Gemstones', 'Edits the parameters of existing prongs between gemstones.')

selectGemstonesInputDef = constants.InputDef(
    constants.ProngsBetweenGemstones.selectGemstonesInputId,
    'Select Gemstones',
    'Select at least 2 gemstones to create prongs between them.'
    )

sizeRatioInputDef = constants.InputDef(
    constants.ProngsBetweenGemstones.sizeRatioInputId,
    'Prong Size Ratio',
    "Prong size relative to average gemstone diameter.\nFrom 0.1 to 0.5 of average diameter (0.3 default)."
    )

heightRatioInputDef = constants.InputDef(
    constants.ProngsBetweenGemstones.heightRatioInputId,
    'Prong Height Ratio',
    "Prong height relative to average gemstone diameter.\nFrom 0.1 to 1.0 of average diameter (0.3 default)."
    )

uniformityInputDef = constants.InputDef(
    constants.ProngsBetweenGemstones.uniformityInputId,
    'Uniformity',
    'Blends prong size and height toward the median values of all generated prongs.\nFrom 0.0 to 1.0, where 0.0 keeps variation and 1.0 makes all generated prongs equal (0.5 default).'
    )

widthBetweenProngsRatioInputDef = constants.InputDef(
    constants.ProngsBetweenGemstones.widthBetweenProngsRatioInputId,
...
maxGapInputDef = constants.InputDef(
    constants.ProngsBetweenGemstones.maxGapInputId,
    'Max Gap',
    "Maximum gap between gemstones for prong creation.\nProngs connect gemstones closer than this distance (0.5 mm default)."
    )

weldDistanceInputDef = constants.InputDef(
    constants.ProngsBetweenGemstones.weldDistanceInputId,
    'Weld Distance',
...
def run(panel: adsk.core.ToolbarPanel):
    ...
    try:
        ...
        _customFeatureDefinition = adsk.fusion.CustomFeatureDefinition.create(constants.ProngsBetweenGemstones.commandId, constants.ProngsBetweenGemstones.id, RESOURCES_FOLDER)
        _customFeatureDefinition.editCommandId = constants.ProngsBetweenGemstones.editCommandId
...
def stop(panel: adsk.core.ToolbarPanel):
    ...
    try:
        control = panel.controls.itemById(constants.ProngsBetweenGemstones.createCommandId)
        if control:
            control.deleteMe()

        commandDefinition = _ui.commandDefinitions.itemById(constants.ProngsBetweenGemstones.createCommandId)
        if commandDefinition:
            commandDefinition.deleteMe()

        commandDefinition = _ui.commandDefinitions.itemById(constants.ProngsBetweenGemstones.editCommandId)
        if commandDefinition:
            commandDefinition.deleteMe()
    except:
...
class CreateCommandCreatedHandler(adsk.core.CommandCreatedEventHandler):
    ...
    and value inputs for prong size, height, uniformity, and max gap, and connects event handlers for validation,
    ...
    def notify(self, args):
        try:
            global _gemstonesSelectionInput, _sizeRatioValueInput, _heightRatioValueInput, _uniformityValueInput, _widthBetweenProngsRatioValueInput, _maxGapValueInput, _weldDistanceValueInput
            ...
            uniformity = adsk.core.ValueInput.createByReal(DEFAULT_UNIFORMITY)
            _uniformityValueInput = inputs.addValueInput(uniformityInputDef.id, uniformityInputDef.name, '', uniformity)
            _uniformityValueInput.tooltip = uniformityInputDef.tooltip
...
class EditCommandCreatedHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self, args):
        try:
            global _editedCustomFeature, _gemstonesSelectionInput, _sizeRatioValueInput, _heightRatioValueInput, _uniformityValueInput, _widthBetweenProngsRatioValueInput, _maxGapValueInput, _weldDistanceValueInput
            ...
            uniformityParameter = parameters.itemById(uniformityInputDef.id)
            uniformityExpression = uniformityParameter.expression if uniformityParameter is not None else str(LEGACY_UNIFORMITY)
            uniformity = adsk.core.ValueInput.createByString(uniformityExpression)
            _uniformityValueInput = inputs.addValueInput(uniformityInputDef.id, uniformityInputDef.name, '', uniformity)
            _uniformityValueInput.tooltip = uniformityInputDef.tooltip
...
class ValidateInputsHandler(adsk.core.ValidateInputsEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            if not _uniformityValueInput.isValidExpression:
                eventArgs.areInputsValid = False
                return

            if not (_uniformityValueInput.value >= 0.0 and _uniformityValueInput.value <= 1.0):
                eventArgs.areInputsValid = False
                return
...
class ExecutePreviewHandler(adsk.core.CommandEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            uniformity = _uniformityValueInput.value
            ...
            prongs = createBodies(gemstones, sizeRatio, heightRatio, uniformity, widthBetweenProngsRatio, maxGap, weldDistance)
            if not prongs:
                return
...
class CreateExecuteHandler(adsk.core.CommandEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            prongs = createBodies(gemstones, _sizeRatioValueInput.value, _heightRatioValueInput.value, _uniformityValueInput.value, _widthBetweenProngsRatioValueInput.value, _maxGapValueInput.value, _weldDistanceValueInput.value)
            if not prongs:
                eventArgs.executeFailed = True
            ...
            uniformity = adsk.core.ValueInput.createByString(_uniformityValueInput.expression)
            customFeatureInput.addCustomParameter(uniformityInputDef.id, uniformityInputDef.name, uniformity, '', True)
...
class EditExecuteHandler(adsk.core.CommandEventHandler):
    ...
    def notify(self, args):
        ...
        try:
            ...
            uniformityParameter = _editedCustomFeature.parameters.itemById(uniformityInputDef.id)
            if uniformityParameter is not None:
                uniformityParameter.expression = _uniformityValueInput.expression
...
def createBodies(gemstones: list[adsk.fusion.BRepBody], sizeRatio: float, heightRatio: float, uniformity: float, widthBetweenProngsRatio: float, maxGap: float = 0.05, weldDistance: float = 0.03) -> list[adsk.fusion.BRepBody] | None:
    ...
    Args:
        ...
        uniformity: Blends prong size and height toward the median values of all generated prongs.
    ...
    try:
        ...
        prongInfos = createProngInfosFromConnections(connections, gemstoneInfos, sizeRatio, heightRatio, uniformity, widthBetweenProngsRatio, weldDistance)
...
def updateFeature(customFeature: adsk.fusion.CustomFeature) -> bool:
    ...
    try:
        ...
        uniformityParameter = customFeature.parameters.itemById(uniformityInputDef.id)
        uniformity = uniformityParameter.value if uniformityParameter is not None else LEGACY_UNIFORMITY
